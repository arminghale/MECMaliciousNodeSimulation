...
SIMULATION_CONFIG = {
    "bounds": (0, 0, 100, 100),  # (x_min, y_min, x_max, y_max)
    "timesteps": 500,
    "mobility_model": "random_waypoint",
    "update_frequency": 1,  # Update every timestep
}

def random_waypoint(user: Dict, bounds: Tuple = (0, 0, 100, 100), boundary_handling: str = "bounce") -> None:
    try:
...
def get_nearby_servers(user: Dict, edge_servers: List[Dict], range_distance: float) -> List[Dict]:
...
def generate_edge_servers(num_servers: int = 500, faulty_percentage: float = 10.0) -> List[Dict]:
    servers = []
    server_tiers = [
    ...
    # Calculate number of faulty servers
    num_faulty = int(num_servers * faulty_percentage / 100.0)
    
    # Randomly select which server indices will be faulty
    faulty_indices = set(random.sample(range(num_servers), num_faulty))
    
    x_min, y_min, x_max, y_max = SIMULATION_CONFIG["bounds"]
    
    for i in range(1, num_servers + 1):
        tier = server_tiers[i % len(server_tiers)]
        is_faulty = (i - 1) in faulty_indices  # i is 1-indexed
        ...
        servers.append({
            "attributes": {
                ...
                "power_consumption": 0,
                "active": True,
                "fault_type": "processing_delay_attack" if is_faulty else None,
            },
            "relationships": {}
        })
...
def generate_service_requests(applications: List[Dict], users: List[Dict], num_requests: int = 10000) -> List[Dict]:
    requests = []
...
def create_dataset(
    ...
    seed: int = 42,
    faulty_percentage: float = 10.0
) -> Dict:
    random.seed(seed)
    
    edge_servers = generate_edge_servers(num_edge_servers, faulty_percentage)
