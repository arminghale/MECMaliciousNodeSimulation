...
def generate_edge_servers(num_servers: int = 500, malicious_percentage: float = 10.0) -> List[Dict]:
    """
    Generate edge servers with configurable malicious percentage.
    
    Args:
        num_servers: Total number of edge servers to generate
        malicious_percentage: Percentage of servers that should be malicious (0-100)
    """
    servers = []
    server_tiers = [
    ...
    for i in range(1, num_servers + 1):
        ...
        servers.append({
            "attributes": {
                ...
                "power_consumption": 0,
                "active": True,
                "fault_type": "selective_service_rejection" if is_malicious else None,
            },
            "relationships": {}
        })
...
def create_dataset(
    ...
    seed: int = 42,
    malicious_percentage: float = 10.0
) -> Dict:
    random.seed(seed)
